...
class Window(Tk):
    def __init__(self):
        ...
        Label(self, text="- Key length must be 16, 24 or 32 bytes", font=("Ubuntu", 7)).grid(row=8, column=0, columnspan=2, padx=0, pady=0, sticky=W)
        Label(self, text="- Initial vector length must be 16, 24 or 32 bytes", font=("Ubuntu", 7)).grid(row=9, column=0, columnspan=2, padx=0, pady=0, sticky=W)
    ...
    def getCipher(self, key, iv, mode):
        ...
        if mode == "ECB":
            return AES.new(key=bytes(key, "utf-8"), mode=AES.MODE_ECB)
        else:
            if len(iv) != 16 and len(iv) != 24 and len(iv) != 32:
                return None
            if mode == "CBC":
                mode = AES.MODE_CBC
            elif mode == "CFB":
                ...
                mode = AES.MODE_OFB
            return AES.new(key=bytes(key, "utf-8"), mode=mode, iv=bytes(iv, "utf-8"))
